...
public class RegistrationWindowController implements Initializable {
    ...
    private void guardar(ActionEvent event) {
        NaturalPersonDAO nuevaPersona = new NaturalPersonDAO();
        CompanyDAO nuevaEmpresa = new CompanyDAO();
        ...
        errores();
        mensaje.showAndWait().ifPresent(response -> {//siempre y cuando el boton del mensaje de registro exitoso sea presionado
            if (response == ButtonType.OK) {
                abrirMenuPrincipal(event);//cambiar de ventana
            }
        });
